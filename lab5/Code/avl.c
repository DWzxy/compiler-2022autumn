...
void table_init()
{
	...
	//预先添加read和write函数
	Type *func_type = new_type(BASIC);
	func_type->basic = Basic_int; //返回值
	Type *type = new_type(FUNC);
	type->func.func_type = func_type; //类型为函数
	ListNode *read = new_listnode("read", type);
	ListNode *write = new_listnode("write", type);
	write->type->func.para_num = 1; // write函数接收一个参数
	write->type->func.para = new_listnode("x", func_type);
	insert_listnode(read, &define_stack[define_top]);
	insert_listnode(write, &define_stack[define_top]);
}
