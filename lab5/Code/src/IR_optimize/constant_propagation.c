                                      ...
                                      Map_IR_var_CPValue *fact)
{
    ...
    else if (stmt->stmt_type == IR_OP_STMT)
    {
         ...
         */
        CPValue rs_val = calculateValue(IR_op_type, rs1_val, rs2_val);
        Fact_update_value(fact, def, rs_val);
        //   TODO();
    }
    else
    { // Other Stmt with new_def
        ...
        if (def != IR_VAR_NONE)
        {
             ...
             * Fact_update_value/Fact_meet_value?(...);
             */
            if (stmt->stmt_type == IR_READ_STMT)
            {
                // s等号右侧为read函数，那么x的状态为UNDEF
                IR_read_stmt *read_stmt = (IR_read_stmt *)stmt;
                IR_var def = read_stmt->rd;
                CPValue use_val = get_UNDEF();
                Fact_update_value(fact, def, use_val);
            }
            if (stmt->stmt_type == IR_LOAD_STMT)
            {
                // s等号右侧为load函数，那么x的状态为UNDEF
                IR_read_stmt *load_stmt = (IR_read_stmt *)stmt;
                IR_var def = load_stmt->rd;
                CPValue use_val = get_UNDEF();
                Fact_update_value(fact, def, use_val);
            }
            if (stmt->stmt_type == IR_CALL_STMT)
            {
                // s等号右侧为call函数，那么x的状态为UNDEF
                IR_read_stmt *call_stmt = (IR_read_stmt *)stmt;
                IR_var def = call_stmt->rd;
                CPValue use_val = get_UNDEF();
                Fact_update_value(fact, def, use_val);
            }
            //  TODO();
